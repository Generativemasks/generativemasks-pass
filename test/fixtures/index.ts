...
export interface Contracts {
  NFT: ERC721Mock;
  GMsDerivative: GenerativemasksDerivative;
...
export const setupIntegration = deployments.createFixture(async ({ ethers }) => {
  ...
  const GMsPassFactory = await ethers.getContractFactory("GenerativemasksDerivative");
  const GMsDerivative = (await GMsPassFactory.deploy(
    "https://example.com/",
    "GMsD",
    "GMsD",
    nftAddress,
